...
## 5. P0/P1 배포 전략 — 참고용 (P1은 후순위)

> **현재 방침:** P0 (나만 쓰는 것)에 집중한다. P1 (남도 쓰게 하는 것)은 P0가 충분히
> 다듬어진 후 착수한다. 아래 내용은 P1 착수 시 참고할 아키텍처 설계이며,
> 지금 당장의 구현 판단에는 영향을 주지 않는다.
...
> P1 관련 작업은 "Appendix: P1 체크리스트"로 분리. 지금은 **내가 쓰기 편한 대시보드**만 만든다.
...
| 리스크 | 심각도 | 대응 |
|-------|-------|------|
| Next.js + Python 이종 스택 복잡도 | 중 | `web/` 서브디렉터리 격리. 개발 시 2개 터미널 (`muninn --transport http` + `npm run dev`) |
...
| CORS 보안 | 낮 | 개발: `localhost:3000` 허용 |
...
| 리스크 | 심각도 | 대응 |
|-------|-------|------|
| Static export가 SPA 라우팅을 제대로 지원하는지 | 중 | `SPAStaticFiles` fallback 패턴으로 해결 |
| Wheel 크기 증가 (정적 파일 번들) | 낮 | Next.js static export ≈ 2-5MB. 절대값은 무시 가능 |
| CI에서 Node.js + Python 듀얼 빌드 | 중 | GitHub Actions에서 Node.js + Python 둘 다 설정 필요 |
| `importlib.resources` 경로 해석 | 낮 | Python 3.11+ 기준 안정적 |
| 프론트엔드 빌드 크기 | 낮 | shadcn/ui는 tree-shakeable. 인증 뒤에 있어 초기 로드 중요도 낮음 |
...
## Appendix: P1 체크리스트 (후순위 — P0 완료 후 착수)

> P0 대시보드가 내가 쓰기에 충분히 편해진 후, 아래 항목을 순서대로 진행한다.
> 자세한 아키텍처 설계는 Section 5 참고.
...
- [ ] `hatch_build.py`: CI에서 `npm run build` 자동 실행
- [ ] Dockerfile 업데이트: Node.js 빌드 스테이지 추가 → 정적 파일 복사
- [ ] docker-compose 업데이트: `--dashboard` 플래그 추가
- [ ] README: "Dashboard Setup" 섹션 (`pip install muninn-mcp[dashboard]`)
- [ ] CORS 프로덕션 설정 (same-origin이면 불필요)
- [ ] 온보딩 위자드 (첫 실행 시 MCP 설정 가이드)
- [ ] 사용자 문서 (depth 시스템 설명, 시작 가이드)
