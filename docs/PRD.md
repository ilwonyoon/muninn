...
**218 tests passing** (2026-02-24 기준)

**test_store.py — ✅ 완료 (87 tests):**
- [x] 프로젝트 CRUD (생성, 조회, 수정, 상태 변경)
- [x] 메모리 저장 후 recall 왕복
- [x] active만 recall, paused 제외 확인
- [x] superseded 메모리 필터링
- [x] FTS5 검색 (한글/영어)
- [x] character budget 초과 시 truncation
- [x] 동시 쓰기 (multiprocessing으로 2개 프로세스 동시 write)
- [x] 메모리 ID prefix 매칭 (update/delete)

**test_tools.py — ✅ 완료 (35 tests):**
- [x] 각 도구의 파라미터 검증 (필수 파라미터 누락 시 에러)
- [x] 존재하지 않는 프로젝트 recall 시 동작
- [x] 빈 결과 (메모리 0개) 처리
- [x] 특수문자 포함 프로젝트 ID 처리
- [x] Usage logging (JSONL 기록 검증)

**test_memory_quality.py — ✅ 완료 (30 tests):**
- [x] Depth 0 항상 로드 (depth 0/1/3에서)
- [x] Recall 정렬 순서 (depth ASC, updated_at DESC)
- [x] Character budget 강제 적용
- [x] Content validation (빈 문자열, 공백만)
- [x] Depth validation (범위 0-3)
- [x] Tag hygiene (빈 태그 거부, 필터링)
- [x] Supersede chain (recall/search에서 숨김)
- [x] Multi-project recall (active만, archived 제외)
- [x] FTS5 검색 품질 (stemming, limit)

**test_server.py — ✅ 완료 (12 tests):**
- [x] CLI 파라미터 파싱 (transport, host, port)
- [x] MCP 인스턴스 생성 (6개 도구 등록)

**test_auth.py — ✅ 완료 (24 tests):**
- [x] Bearer token 인증 (valid/invalid/missing)
- [x] 잘못된 prefix 거부 (lowercase, Basic, raw)
- [x] Constant-time comparison (hmac.compare_digest)
- [x] Whitespace stripping

**test_github_sync.py — ✅ 완료 (7 tests):**
- [x] GitHub API 응답 파싱 (commits, issues, PRs)
- [x] Issues에서 PR 제외 필터링
- [x] 메모리 저장 (source='github')
- [x] 두 번째 sync가 첫 번째를 supersede
- [x] 도구 함수 end-to-end

**test_formatter.py — ❌ 미구현:**
- [ ] depth별 출력 포맷 검증
...
브라우저에서 확인:
- [x] 등록된 도구 목록 (muninn_save, recall, search, status, manage, sync)
- [ ] 각 도구에 파라미터 입력 → 직접 호출 → 응답 JSON 확인
- [ ] 에러 케이스 (잘못된 파라미터, 없는 프로젝트)

*참고: MCP Inspector 테스트는 수동. 도구 등록은 test_server.py에서 자동 검증됨.*

### Level 3: Real Client Integration (진짜 AI 도구에서 동작하는가)

구현 완료 후 수동으로 실행하는 체크리스트.

**Claude Code 연결 테스트 — ✅ 완료:**
```
[x] claude mcp add muninn -- python -m muninn.server 성공
[x] "aido 프로젝트 만들어줘" → muninn_save 호출됨
[x] "내 프로젝트 보여줘" → muninn_status 호출됨
[x] "aido에 대해 알려줘" → muninn_recall 호출됨
[x] "gamification 관련 저장한 거 찾아줘" → muninn_search 호출됨
[x] "glpuri를 paused로 바꿔줘" → muninn_manage 호출됨
```

**Cross-tool 테스트 (Phase 1 핵심) — 🔲 미완료:**
```
...
**Cross-platform 테스트 (Phase 2) — 🟡 부분 완료:**
```
[x] Streamable HTTP 서버 실행 → MCP Inspector로 연결
[x] ngrok 터널 → Claude Web (claude.ai)에서 연결 → save/recall 동작
[x] iPhone Claude에서 연결 → save/recall 동작
...
**자동 사용 로그 — ✅ 구현됨:**
```python
# tools.py _log_usage() — 도구 호출마다 기록
# ~/.local/share/muninn/usage.jsonl에 JSONL 형식
{"ts": "2026-02-24T...", "tool": "muninn_save", "project": "muninn", "depth": 1}
```

**1주일 후 로그 분석 — 🔲 데이터 축적 필요:**

| 질문 | 로그에서 확인 | 상태 |
|------|-------------|------|
| 하루 recall 횟수 | `tool="muninn_recall"` 카운트 | 🔲 미측정 |
| 가장 많이 조회하는 프로젝트 | project별 그룹핑 | 🔲 미측정 |
| save vs recall 비율 | save가 극단적으로 적으면 저장 UX 문제 | 🔲 미측정 |
| search 사용 빈도 | 0이면 FTS5 구현이 불필요했던 것 | 🔲 미측정 |
...
**상태: 🔲 미구현** — 벤치마크 테스트 아직 작성하지 않음.

```python
# 목표:
# - recall: < 100ms (5 projects × 수백 memories)
# - search: < 200ms
# - save: < 50ms
```
...
| 순서 | 레벨 | 무엇을 증명 | 언제 | 상태 |
|------|------|-----------|------|------|
| 1 | Unit (L1) | 코드 정확성 | 구현과 동시에 | ✅ 218 tests |
| 2 | MCP Inspector (L2) | 프로토콜 준수 | 서버 완성 직후 | 🟡 부분 |
| 3 | Claude Code 연결 (L3) | 실제 동작 | Day 1 마무리 | ✅ 완료 |
| 4 | Cross-tool (L3) | 크로스툴 증명 | Day 2 | 🟡 부분 |
| 5 | Dogfooding (L4) | 실용성 증명 | Week 1-2 | 🔲 미시작 |
| 6 | Performance (L5) | 속도 증명 | Phase 1 완료 후 | 🔲 미시작 |
