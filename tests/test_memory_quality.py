...
- Recall respects character budget
...
from muninn.models import Memory, validate_memory_content
...
class TestCharacterBudget:
    ...
    def test_budget_drops_excess_memories(self, store, project):
        store.save_memory(project, "A" * 100)
        store.save_memory(project, "B" * 100)
        store.save_memory(project, "C" * 100)

        # Budget of 250 should fit first two (200 chars) but drop the third
        memories, stats = store.recall(project_id=project, max_chars=250)
    ...
    def test_budget_zero_loads_nothing(self, store, project):
        store.save_memory(project, "Some content.")
        memories, stats = store.recall(project_id=project, max_chars=0)
        assert stats["memories_loaded"] == 0

    def test_large_budget_loads_all(self, store, project):
        for i in range(10):
            store.save_memory(project, f"Memory {i}")

        memories, stats = store.recall(project_id=project, max_chars=100_000)
...
class TestContentValidation:
    ...
    def test_valid_content_accepted(self, store, project):
        mem = store.save_memory(project, "Valid content.")
...
class TestTagHygiene:
    ...
    def test_valid_tags_stored_and_recalled(self, store, project):
        store.save_memory(project, "Tagged memory.", tags=["decision", "api"])
        memories, _ = store.recall(project_id=project)
        mems = memories.get(project, [])
        assert len(mems) == 1
        assert set(mems[0].tags) == {"decision", "api"}

    def test_tag_filter_recall(self, store, project):
        store.save_memory(project, "About auth.", tags=["decision", "auth"])
        store.save_memory(project, "About api.", tags=["decision", "api"])

        memories, _ = store.recall(project_id=project, tags=["auth"])
        mems = memories.get(project, [])
        assert len(mems) == 1
...
class TestSupersedeChains:
    ...
    def test_superseded_memory_hidden_from_recall(self, store, project):
        old = store.save_memory(project, "Old decision.")
        new = store.save_memory(project, "Updated decision.")
    ...
    def test_superseded_memory_hidden_from_search(self, store, project):
        old = store.save_memory(project, "SQLite WAL mode chosen.")
        new = store.save_memory(project, "SQLite WAL mode confirmed.")
        store.supersede_memory(old.id, new.id)
    ...
    def test_deleted_memory_hidden(self, store, project):
        mem = store.save_memory(project, "To be deleted.")
...
class TestDeduplicationAwareness:
    ...
    def test_search_finds_similar_content(self, store, project):
        store.save_memory(project, "Using SQLite with WAL mode for concurrency.")
        store.save_memory(project, "Database: PostgreSQL for production.")
    ...
    def test_search_across_projects(self, store):
        ...
        store.save_memory("proj1", "OAuth 2.0 implementation.")
        store.save_memory("proj2", "OAuth token rotation.")
...
class TestMultiProjectRecall:
    ...
    def test_recall_all_active_projects(self, store):
        ...
        store.save_memory("active1", "Memory A.")
        store.save_memory("active2", "Memory B.")

        memories, stats = store.recall()
    ...
    def test_recall_excludes_archived_projects(self, store):
        ...
        store.save_memory("live", "Active memory.")
        store.save_memory("dead", "Archived memory.")
        store.update_project("dead", status="archived")

        memories, _ = store.recall()
    ...
    def test_recall_specific_project_ignores_status(self, store):
        ...
        store.save_memory("arch", "Still accessible.")
        store.update_project("arch", status="archived")

        memories, _ = store.recall(project_id="arch")
...
class TestFTSSearchQuality:
    ...
    def test_partial_word_match_via_porter_stemming(self, store, project):
        store.save_memory(project, "Implementing authentication flow.")
